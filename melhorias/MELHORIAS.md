...
- [ ] **2. Tag `</img>` invalida** — Em todas as 5 paginas, a tag do logo usa `<img ...></img>`. A tag `<img>` e self-closing em HTML5 (nao precisa de `</img>`).
...
- [ ] **6. Links HTTP em vez de HTTPS** — Varios links externos usam `http://` (Lattes, UFSCar, links de PDF do dm.ufscar.br). Devem usar `https://` quando possivel.
...
- [ ] **8. Falta `meta description`** — Nenhuma pagina tem `<meta name="description">`, o que prejudica SEO.
- [ ] **9. Falta `alt` em varias imagens** — `Lattesbranco.png`, `Ufscar.gif`, o mapa do ClustrMaps e outras imagens nao tem atributo `alt` descritivo (acessibilidade).

## Melhorias de Estrutura/Codigo

- [ ] **10. Header e footer duplicados nas 5 paginas** — Qualquer mudanca no menu precisa ser replicada manualmente. Considerar usar includes ou static site generator.
- [ ] **11. Excesso de `!important` no CSS** — Pelo menos 8 usos em `style.css`. A maioria pode ser eliminada reordenando os CSS e usando seletores mais especificos.
- [ ] **12. Estilos inline excessivos** — Muitos elementos tem `style="..."` diretamente no HTML. Migrar para classes no `style.css`.
- [x] **13. CSS `style.css:88` — seletor errado** — `.table-styleth` esta faltando um espaco (deveria ser `.table-style th`).
- [ ] **14. Falta `.gitignore`** — Nao existe um `.gitignore` para excluir `.DS_Store`, etc.
...
- [ ] **18. Foto na index muito pequena** — `.img-fluid { max-width: 25% !important; }` faz a foto ficar minuscula em mobile. Usar um tamanho responsivo melhor (ex: `max-width: 300px`).
...
- [ ] **21. Links nas tabelas com baixo contraste** — Na `research.html`, links `#2a42ff` sobre fundo `#718af0` sao dificeis de ler.
- [ ] **22. Layout vazio em `miscellaneous.html`** — A coluna direita tem poucos itens e muito espaco vazio.
- [ ] **23. Tabelas nao ideais para mobile** — Cards do Bootstrap seriam mais responsivos para listar publicacoes e alunos.
